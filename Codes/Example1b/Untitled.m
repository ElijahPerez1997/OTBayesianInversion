...
a0=15000;b0=0.3;%shape and rate of prior for s
...
theta3=zeros(1,M);theta3_0=2;theta3(1)=theta3_0;%Multiplicitive hyperparameter
%Memory prellocation
...
%     thetas=mvnrnd([theta1(i),theta2(i),theta3(i)],[0.001,0.001,0.2]);%proposal distribution for theta 1
%     theta1_s=thetas(:,1);
%     theta2_s=thetas(:,2);
%     theta3_s=thetas(:,3);
    
    
thetas=mvnrnd([theta1(i),theta2(i)],[0.00001,0.00001]);
    theta1_s=thetas(:,1);
    theta2_s=thetas(:,2);
    theta3_s=theta3(i)+unifrnd(-r/2,r/2);
    if theta3_s<0
           theta3_s=5-mod(1,theta3_s);
    elseif theta3_s>5
           theta3_s=theta3_s-5;
    end
  ...
  for k=1:n1
    ...
    DSD_s=1000*theta3_s*DSD_s;
   
    
    li=-s(i)*DSD_i; %Log liklihood with fi 
    ls=-s(i)*DSD_s; %Log likelihood with fs
    ...
    if 0>theta3(i) || theta3(i)>5 || 0>theta3_s || theta3_s>5
        alpha=-1E10;
    end
...
theta3_burn=theta3(burn:thin:end);
