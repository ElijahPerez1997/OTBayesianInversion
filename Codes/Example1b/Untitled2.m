...
a0=2500000;b0=0.3;%shape and rate of prior for s
...
while i<M
    ...
    thetas=mvnrnd([theta1(i),theta2(i)],[.00001,.00001]);%proposal distribution for theta 1
    theta1_s=thetas(:,1);
    theta2_s=thetas(:,2);
  %u=@(x,t,x0,a) 0.5*a*((exp(-100*((x-t-x0-0.5).^2))+exp(-100*((x-t-x0).^2))+exp(-100*((x-t-x0+0.5).^2)))+(exp(-100*((x+t-x0-0.5).^2))+exp(-100*((x+t-x0).^2))+exp(-100*((x+t-x0+0.5).^2))));
  for k=1:n1
