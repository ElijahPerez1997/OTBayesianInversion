...
g=zeros(1,n);%Memory prellocation
f=zeros(1,n);
fi=zeros(1,n);
gi=zeros(1,n);
fs=zeros(1,n);
...
for i=1:n
    ...
    f(i)=theta1_star+theta2_star*x(i);
    g(i)=theta1_star+theta2_star*x(i)+eps;
end
 %gi=g/sum(g);
%f=f/sum(f);
%g=g/sum(g);
...
M=2.0E4;%Number of samples drawn
burn=round(M/4);%Number of values burned
thin=2;
...
r=0.4;%Support of proposal
s=zeros(1,M-1);%Memory prellocation

while i<M
    for j=1:n
    fi(j)=theta1(i)+theta2(i)*x(j);
    end
    c=min([fi g]);
    if c<0
        c=-c+1;
    ...
    b_aprox=b0+Wasserstein(fi,gi,x);  
    %b_aprox=0.5*sum(abs(g-theta1(i)-theta2(i).*x).^2);
    s(i)=gamrnd(a_aprox,1/b_aprox);
    
    
    theta1_s=unifrnd(theta1(i)-(r/2),theta1(i)+(r/2));%proposal distributions for theta 1 and 2
    theta2_s=unifrnd(theta2(i)-(r/2),theta2(i)+(r/2));
    
    for j=1:n
    fs(j)=theta1_s+theta2_s*x(j);
    end
    fs=fs/sum(fs);
   
    %ls=-0.5*s(i)*sum(abs(g-theta1_s-theta2_s*x).^2);
    %li=-0.5*s(i)*sum(abs(g-theta1(i)-theta2(i)*x).^2);
    
    li=-s(i)*Wasserstein(fi,gi,x); %Log liklihood with fi 
    ls=-s(i)*Wasserstein(fs,gi,x); %Log likelihood with fs
    %Note: Contants for the log liklihoods cancle out and therefore are not
    %included
    
    %Log priors with constants cancled
...
mu=V*(inv(V0)*mu0+x2d'*g');
a_star=a0+n/2;
b_star=b0+0.5*(mu0'*inv(V0)*mu0+g*g'-mu'*inv(V)*mu);
